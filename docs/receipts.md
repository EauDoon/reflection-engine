...
Packet reads use one regular-file descriptor and stop at 8 MiB plus one overflow-detection byte. Files that grow beyond that limit are rejected before hashing. Hashing uses the exact bytes read, without text decoding or newline normalization.
