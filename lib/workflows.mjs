...
import { reviewPlan, validateReview, reviewSummary } from './review.mjs';
import { planExperiment, experimentSummary } from './experiment.mjs';

export const WORKFLOW_COMMANDS = ['selection-plan', 'import-text', 'merge', 'select', 'inspect', 'preview', 'draft-report', 'pack-run', 'review-plan', 'validate-review', 'review-summary', 'plan-experiment', 'review-experiment'];
export function runWorkflow(args) {
  const [command, ...paths] = args;
  if (command === 'selection-plan') {
