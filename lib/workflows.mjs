...
import { planExperiment, experimentSummary, validateExperiment, appendObservation } from './experiment.mjs';
import { questions } from './packet.mjs';
import { compareDetails } from './compare.mjs';
import { previewRedaction } from './redact.mjs';

export const WORKFLOW_COMMANDS = ['append-observation', 'validate-experiment', 'preview-redaction', 'compare-details', 'export-accepted', 'review-gaps', 'evidence-map', 'questions', 'selection-plan', 'import-text', 'merge', 'select', 'inspect', 'preview', 'draft-report', 'pack-run', 'review-plan', 'validate-review', 'review-summary', 'plan-experiment', 'review-experiment'];
export function runWorkflow(args) {
  const [command, ...paths] = args;
  if (command === 'append-observation') {
    requireThat(paths.length === 5, 'Usage: append-observation <run.json> <review.json> <experiment.json> <entry.json> <new-experiment.json>');
    writeJSONNew(paths[4], appendObservation(readJSON(paths[0]), readJSON(paths[1]), readJSON(paths[2]), readJSON(paths[3])));
    console.log('Observation appended to a new validated record. Dates and outcomes are self-reported; no action was executed.');
  } else if (command === 'validate-experiment') {
    requireThat(paths.length === 3, 'Usage: validate-experiment <run.json> <review.json> <experiment.json>');
    validateExperiment(readJSON(paths[0]), readJSON(paths[1]), readJSON(paths[2]));
    console.log('Experiment structure and run binding are valid. All observations and outcomes remain self-reported.');
  } else if (command === 'preview-redaction') {
    requireThat(paths.length === 3, 'Usage: preview-redaction <corpus.json> <rules.json> <new-preview.json>');
    writeJSONNew(paths[2], previewRedaction(readJSON(paths[0]), readJSON(paths[1])));
    console.log('Literal redaction preview created. Zero matches and metadata require manual review.');
  } else if (command === 'compare-details') {
    requireThat(paths.length === 3, 'Usage: compare-details <before-run.json> <after-run.json> <new-diff.json>');
    writeJSONNew(paths[2], compareDetails(readJSON(paths[0]), readJSON(paths[1])));
    console.log('Field-level comparison created. Changes do not measure truth or personal progress.');
  } else if (command === 'export-accepted') {
