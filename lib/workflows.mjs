...
export const WORKFLOW_COMMANDS = ['compare-details', 'export-accepted', 'review-gaps', 'evidence-map', 'questions', 'selection-plan', 'import-text', 'merge', 'select', 'inspect', 'preview', 'draft-report', 'pack-run', 'review-plan', 'validate-review', 'review-summary', 'plan-experiment', 'review-experiment'];
export function runWorkflow(args) {
  const [command, ...paths] = args;
  if (command === 'compare-details') {
