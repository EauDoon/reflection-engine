...
import { reviewPlan, validateReview, reviewSummary, reviewGaps } from './review.mjs';
import { planExperiment, experimentSummary } from './experiment.mjs';
import { questions } from './packet.mjs';

export const WORKFLOW_COMMANDS = ['review-gaps', 'evidence-map', 'questions', 'selection-plan', 'import-text', 'merge', 'select', 'inspect', 'preview', 'draft-report', 'pack-run', 'review-plan', 'validate-review', 'review-summary', 'plan-experiment', 'review-experiment'];
export function runWorkflow(args) {
  const [command, ...paths] = args;
  if (command === 'review-gaps') {
