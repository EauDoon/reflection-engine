...
export function validateExperiment(run, review, experiment) {
  validateReview(run, review);
  const chosen = selectedRun(run);
  exact(experiment, ['version', 'run_digest', 'question_id', 'action', 'started', 'observations', 'reviewed', 'decision', 'reason', 'alternative'], 'Experiment');
  ...
  let previous = experiment.started;
...
export function experimentSummary(run, review, experiment) {
  ...
  return '# Optional experiment review\n\n' + status + '\n\n' +
    'Dates, checks, observations and outcomes are self-reported. This tool does not run the action, schedule a reminder, infer causation, or measure personal growth. Keep competing explanations visible.\n' + warning + '\n' +
