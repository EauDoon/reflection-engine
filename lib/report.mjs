...
export function draftReport(corpus, config = DEFAULT_CONFIG) {
  const selected = configure(corpus, config);
  const report = {
    version: 1, source_ids: selected.corpus.sources.map(source => source.id),
    answers: selected.ids.map(id => ({
      id, status: 'insufficient evidence', confidence: 1, conclusion: '', evidence: [],
      counterevidence: 'No conclusion has been made. Review counterexamples before making one.',
      alternative: 'This is an unreviewed draft, not an interpretation of the supplied sources.',
      action: { step: 'Do not start an experiment from this empty draft.', check: 'Review chosen evidence and the answer contract first.', stop: 'Leave the question unanswered if the evidence is insufficient.' }
    }))
  };
  return validateReport(report, corpus, config);
}
