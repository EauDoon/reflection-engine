...
export function buildPacket(corpus, ids = [4,11,17], format = 'markdown') {
  ...
  const prompt = readFileSync(new URL('../Reflection-Engine-Bounded.md', import.meta.url), 'utf8').replace(/\r\n/g,'\n').split('## Questions')[0].trimEnd();
  const contract = format === 'json' ? readFileSync(new URL('../docs/report-contract.md', import.meta.url), 'utf8').replace(/\r\n/g,'\n') : '';
  requireThat(['markdown','json'].includes(format), 'Invalid output format');
