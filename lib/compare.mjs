...
export function compareDetails(before, after) {
  const left = selectedRun(before), right = selectedRun(after);
  const changes = (previous, current) => {
    const a = new Map(previous.map(item => [item.id, item])), b = new Map(current.map(item => [item.id, item]));
    return {
      added: [...b.keys()].filter(id => !a.has(id)), removed: [...a.keys()].filter(id => !b.has(id)),
      changed: current.filter(item => a.has(item.id)).map(item => ({ id: item.id,
        fields: Object.keys(item).filter(key => !isDeepStrictEqual(a.get(item.id)[key], item[key])) })).filter(item => item.fields.length),
      order_changed: !isDeepStrictEqual([...a.keys()].filter(id => b.has(id)), [...b.keys()].filter(id => a.has(id)))
    };
  };
  return { version: 1, before_digest: runDigest(left), after_digest: runDigest(right),
    sources: changes(left.corpus.sources, right.corpus.sources), answers: changes(left.report.answers, right.report.answers),
    config_fields: Object.keys(left.config).filter(key => !isDeepStrictEqual(left.config[key], right.config[key])) };
}
...
export function compareRuns(before, after) {
  ...
  const leftSources=new Map(left.corpus.sources.map(s=>[s.id,s]));
  const rightSources=new Map(right.corpus.sources.map(s=>[s.id,s]));
  const added=[...rightSources.keys()].filter(id=>!leftSources.has(id));
  const removed=[...leftSources.keys()].filter(id=>!rightSources.has(id));
  const edited=[...rightSources.keys()].filter(id=>leftSources.has(id)&&!isDeepStrictEqual(leftSources.get(id),rightSources.get(id)));
  ...
  const rows=[...new Set([...left.ids,...right.ids])].sort((a,b)=>a-b).map(id=> {
    ...
    return `| ${id} | ${a.status} to ${b.status} | ${a.confidence} to ${b.confidence} | ${isDeepStrictEqual(a,b)?'unchanged':'changed'} |`;
  });
